...
describe("isZodTypeEqual", () => {
    ...
    it("should return true when two enum type are equal", () => {
        expect(isZodTypeEqual(z.enum(["one"]), z.enum(["two"]))).toStrictEqual(true);
    });
    it("should return true when a is an enum and b is an optional enum", () => {
        expect(isZodTypeEqual(z.enum(["cool"]).optional(), z.enum(["b"]))).toStrictEqual(true);
    });
...
//# sourceMappingURL=isZodTypeEqual.test.js.map